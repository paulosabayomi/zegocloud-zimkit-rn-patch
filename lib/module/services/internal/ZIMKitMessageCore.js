...
export default class ZIMKitMessageCore {
  ...
  getMessageList(conversationID, conversationType) {
    ...
    return ZIM.getInstance().queryHistoryMessage(conversationID, conversationType, config).then(({
      messageList
    }) => {
  ...
  sendTextMessage(conversationID, conversationType, text, onMessageSent) {
    ...
    return ZIM.getInstance().sendMessage(messageTextObj, toConversationID, conversationType, config, notification).then(({
      message
    }) => {
